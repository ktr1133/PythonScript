...
def get_message_list(service, user_id, query, count):
    messages = []
    try:
        message_ids = (service.users().messages().list(userId=user_id, maxResults=count, q=query).execute())
        ...
        for message_id in message_ids["messages"]:
            ...
            if 'data' in detail['payload']['body']:
                ...
                message["body"] = ""
            # 件名
            message["subject"] = [header["value"] for header in detail["payload"]["headers"] if header["name"] == "Subject"][0]
            # 送信元
            message["from"] = [header["value"] for header in detail["payload"]["headers"] if header["name"] == "From"][0]
...
def main(query="is:unread", count=10):
    ...
    if messages:
        ...
        with open('mails/{}_gmails.csv'.format(dt.datetime.now().strftime('%Y%m%d%H%M')), 'w', newline='', encoding='utf8') as csvfile:
