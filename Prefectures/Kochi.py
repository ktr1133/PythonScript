...
def kochi(self):
    df_BookMarks = pd.read_csv('D://jupyter notebook/source/BookMarksCategorize.csv')
    SearchWord = self
    BookMarkURL = df_BookMarks[df_BookMarks['category']=='D']['URL'][38]
    BookMarkName = df_BookMarks[df_BookMarks['category']=='D']['name'][38]
    df_kc = pd.DataFrame(index=[], columns=['ID', 'Name', 'SearchWord', 'JournalTitle', 'SpeakOrder', 'Date', 'Speaker', 'PoliticalParty', 'Remark'])
    try:
        ...
        if slideList_temp == []:
            ...
            #ﾚｺｰﾄﾞ作成とﾃﾞｰﾀﾌﾚｰﾑに追加
            record = pd.Series(['', Name, SearchWord, '', '', '', '', '', not_applicable], index=df_kc.columns)
            df_kc = df_kc.append(record,ignore_index=True) 
            browser.quit()
        else:
                    ...
                    for i in range(len(parentlist)):
                        ...
                        for ele in Remark_step2:
                            if SearchWord in ele:
                                ...
                                Name = BookMarkName
                                #ﾚｺｰﾄﾞ作成とﾃﾞｰﾀﾌﾚｰﾑに追加
                                record = pd.Series(['', Name, SearchWord, JournalTitle, SpeakOrder, Date, Speaker, '', Remark], index=df_kc.columns)
                                df_kc = df_kc.append(record,ignore_index=True)
        ...
        Name = BookMarkName
        record = pd.Series(['', Name, SearchWord, '', '', '', '', '', error+' '+msg], index=df_kc.columns)
        df_kc = df_kc.append(record,ignore_index=True)
        browser.quit()
