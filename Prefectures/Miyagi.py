...
def miyagi(self):
    ...
    df_myg = pd.DataFrame(index=[], columns=['ID', 'Name', 'SearchWord', 'JournalTitle', 'SpeakOrder', 'Date', 'Speaker', 'PoliticalParty', 'Remark'])
    try:
        ...
        if slideList_temp == []:
            ...
            #ﾚｺｰﾄﾞ作成とﾃﾞｰﾀﾌﾚｰﾑに追加
            record = pd.Series(['', Name, SearchWord, '', '', '', '', '', not_applicable], index=df_myg.columns)
                        ...
                        for ele in Remark_step2:
                            if '\u3000' in SearchWord:
                                ...
                                if SearchWords[0] in ele.text and SearchWords[1] in ele.text:
                                    ...
                                    #ﾚｺｰﾄﾞ作成とﾃﾞｰﾀﾌﾚｰﾑに追加
                                    record = pd.Series(['', Name, SearchWord, JournalTitle, SpeakOrder, Date, Speaker, '', Remark], index=df_myg.columns)
                                    df_myg = df_myg.append(record,ignore_index=True)
