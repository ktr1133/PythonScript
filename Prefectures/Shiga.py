...
def shiga(self):
    ...
    df_shg = pd.DataFrame(index=[], columns=['ID', 'Name', 'SearchWord', 'JournalTitle', 'SpeakOrder', 'Date', 'Speaker', 'PoliticalParty', 'Remark'])
    try:
        ...
        if JURLs_temp1 == []:
            ...
            JURLs_temp1 = browser.find_elements_by_tag_name('table')[1].\
            find_elements_by_css_selector('a[target="HLD_WIN"]')
            ...
            for i in range(len(JURLs_temp1)):
                ...
                for ele in speakers_temp1:
                    ...
                    if '議長' not in Speaker and '委員長' not in Speaker:
                        ...
                        SpeakOrder = num
                        Name = BookMarkName
                        record = pd.Series(['', Name, SearchWord, JournalTitle, SpeakOrder, Date, Speaker, '', Remark], index=df_shg.columns)
                        df_shg = df_shg.append(record,ignore_index=True)
                        num = num+1
                    browser.switch_to_default_content()
                    browser.switch_to.frame('sidebar')
            ...
            for ele2 in JURLs_temp2:
                ...
                sleep(2)
                for j in range(len(JURLs_temp3)):
                    JURLs_temp3 = browser.find_elements_by_css_selector("a[target='HLD_WIN']")
                    JURLs_temp3[j].click()
                    browser.implicitly_wait(5)
                    browser.switch_to.window(browser.window_handles[-1]) #新しく開いたｳｨﾝﾄﾞｳに移動
