...
def ishikawa(self):
    ...
    df_isk = pd.DataFrame(index=[], columns=['ID', 'Name', 'SearchWord', 'JournalTitle', 'SpeakOrder', 'Date', 'Speaker', 'PoliticalParty', 'Remark'])
    try:
            ...
            for i in range(len(JURLs_temp2)):
                ...
                for j in range(len(speakers_temp1)):
                    ...
                    if '議長' not in Speaker and '委員長' not in Speaker:
                        SpeakerID = speakers_temp1[j].find_element_by_tag_name('a').get_attribute('href').split('#')[1]
                        speakers_temp1[j].click()
                        sleep(2)
                        browser.switch_to_default_content()
                        ...
                        SpeakOrder = num
                        Name = BookMarkName
                        record = pd.Series(['', Name, SearchWord, JournalTitle, SpeakOrder, Date, Speaker, '', Remark], index=df_isk.columns)
                        df_isk = df_isk.append(record,ignore_index=True)
                        num = num+1
                        browser.switch_to_default_content()
                        browser.switch_to.frame('sidebar')
