...
def yamagata(self):
    df_BookMarks = pd.read_csv('D://jupyter notebook/source/BookMarksCategorize.csv')
    SearchWord = self
    BookMarkURL = df_BookMarks[df_BookMarks['category']=='D']['URL'][5]
    BookMarkName = df_BookMarks[df_BookMarks['category']=='D']['name'][5]
    df_ymgt = pd.DataFrame(index=[], columns=['ID', 'Name', 'SearchWord', 'JournalTitle', 'SpeakOrder', 'Date', 'Speaker', 'PoliticalParty', 'Remark'])
    try:
        ...
        if slideList_temp == []:
            ...
            #ﾚｺｰﾄﾞ作成とﾃﾞｰﾀﾌﾚｰﾑに追加
            record = pd.Series(['', Name, SearchWord, '', '', '', '', '', not_applicable], index=df_ymgt.columns)
            df_ymgt = df_ymgt.append(record,ignore_index=True) 
            browser.quit()
        else:
                        ...
                        for ele in Remark_step2:
                            if SearchWord in ele:
                                ...
                                if '議長' not in Speaker and '委員長' not in Speaker:
                                    ...
                                    Name = BookMarkName
                                    #ﾚｺｰﾄﾞ作成とﾃﾞｰﾀﾌﾚｰﾑに追加
                                    record = pd.Series(['', Name, SearchWord, JournalTitle, SpeakOrder, Date, Speaker, '', Remark], index=df_ymgt.columns)
                                    df_ymgt = df_ymgt.append(record,ignore_index=True)
        ...
        error = 'error'
        Name = BookMarkName
        record = pd.Series(['', Name, SearchWord, '', '', '', '', '', error], index=df_ymgt.columns)
        df_ymgt = df_ymgt.append(record,ignore_index=True)
        browser.quit()
