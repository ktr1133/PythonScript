...
def akita(self):
    df_BookMarks = pd.read_csv('D://jupyter notebook/source/BookMarksCategorize.csv')
    SearchWord = self
    BookMarkURL = df_BookMarks[df_BookMarks['category']=='D']['URL'][4]
    BookMarkName = df_BookMarks[df_BookMarks['category']=='D']['name'][4]
    df_akt = pd.DataFrame(index=[], columns=['ID', 'Name', 'SearchWord', 'JournalTitle', 'SpeakOrder', 'Date', 'Speaker', 'PoliticalParty', 'Remark'])
    try:
        ...
        if slideList_temp == []:
            ...
            #ﾚｺｰﾄﾞ作成とﾃﾞｰﾀﾌﾚｰﾑに追加
            record = pd.Series(['', Name, SearchWord, '', '', '', '', '', not_applicable], index=df_akt.columns)
            df_akt = df_akt.append(record,ignore_index=True)
            browser.quit()
        else:
            ...
            for i in range(len(slideList)):
                ...
                if parentlist != []:
                    for i in range(len(parentlist)):
                        ...
                        Date = JournalTitle.split('　')[0]+parentlist[i].find_element_by_tag_name('span').text.split(' ')[-2].split('－')[0]
                        ...
                        for ele in Remark_step2:
                            if SearchWord in ele:
                                ...
                                if "◆" in Remark_step3.split('（')[0]:
                                    Speaker_step1 = Remark_step3.split('　')[0]
                                    Speaker = Speaker_step1.split('◆')[1]
                                    SpeakOrder = num
                                ...
                                Name = BookMarkName
                                #ﾚｺｰﾄﾞ作成とﾃﾞｰﾀﾌﾚｰﾑに追加
                                record = pd.Series(['', Name, SearchWord, JournalTitle, SpeakOrder, Date, Speaker, '', Remark], index=df_akt.columns)
                                df_akt = df_akt.append(record,ignore_index=True)
        ...
        error = 'error'
        Name = BookMarkName
        record = pd.Series(['', Name, SearchWord, '', '', '', '', '', error], index=df_akt.columns)
        df_akt = df_akt.append(record,ignore_index=True)
        browser.quit()
